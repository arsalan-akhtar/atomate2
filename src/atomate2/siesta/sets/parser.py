...
__version__ = "1.0"
__email__ = "sr.arsalan.akhtar@gmail.com"
...
class SiestaOutHeaderChunk(SiestaOutChunk):
    ...
    def commit_hash(self) -> str:
        ...
        if line_start == LINE_NOT_FOUND:
            raise SiestaParseError("This file does not appear to be an siesta-output file")
    ...
    def initial_structure(self) -> Structure | Molecule:
        ...
        Using the SIESTA output file recreate the initial structure for
        the calculation.
        """
        ...
        if line_start == LINE_NOT_FOUND:
            raise SiestaParseError("No information about the structure in the chunk.")
    ...
    def n_atoms(self) -> int:
        ...
        if n_atoms is None:
            raise SiestaParseError("No information about the number of atoms in the header.")
        return int(n_atoms)
    ...
    def n_bands(self) -> int | None:
        ...
        if line_start == LINE_NOT_FOUND:
            raise SiestaParseError("No information about the number of Kohn-Sham states in the header.")
    ...
    def n_electrons(self) -> int | None:
        ...
        if line_start == LINE_NOT_FOUND:
            raise SiestaParseError("No information about the number of electrons in the header.")
    ...
    def n_spins(self) -> int | None:
        ...
        if n_spins is None:
            raise SiestaParseError("No information about the number of spin channels in the header.")
        return int(n_spins)
...
class SiestaOutCalcChunk(SiestaOutChunk):
    ...
    def __init__(self, lines: list[str], header: SiestaOutHeaderChunk) -> None:
        """Construct the SiestaOutCalcChunk.

        Args:
            lines (list[str]): The lines used for the structure
            header (.SiestaOutHeaderChunk):  A summary of the relevant information from
                the siesta.out header
        """
    ...
    def energy(self) -> float:
        ...
        if line_ind == LINE_NOT_FOUND:
            raise SiestaParseError("No energy is associated with the structure.")
...
def get_header_chunk(content: str | TextIOWrapper) -> SiestaOutHeaderChunk:
    ...
    return SiestaOutHeaderChunk(header)


def get_siesta_out_chunks(content: str | TextIOWrapper, header_chunk: SiestaOutHeaderChunk) -> Generator:
    """Yield unprocessed chunks (header, lines) for each SiestaOutChunk image.
    ...
    while True:
        ...
        while chunk_end_line not in line or ignore_chunk_end_line:
            ...
            try:
                ...
                break
        yield SiestaOutCalcChunk(chunk_lines, header_chunk)


def check_convergence(chunks: list[SiestaOutCalcChunk], non_convergence_ok: bool = False) -> bool:
...
def read_siesta_header_info_from_content(
    content: str,
) -> tuple[dict[str, list[str] | None | str], dict[str, Any]]:
    """Read the Siesta header information.
...
def read_siesta_header_info(
    ...
    return read_siesta_header_info_from_content(content)


def read_siesta_output_from_content(
    content: str, index: int | slice = -1, non_convergence_ok: bool = False
) -> Structure | Molecule | Sequence[Structure | Molecule]:
    """Read and siesta output file from the content of a file.
    ...
    """
    header_chunk = get_header_chunk(content)
    chunks = list(get_siesta_out_chunks(content, header_chunk))
...
def read_siesta_output(
    filename: str | Path,
    index: int | slice = -1,
    non_convergence_ok: bool = False,
) -> Structure | Molecule | Sequence[Structure | Molecule]:
    """Import SIESTA output files with all data available.

    Includes all structures for relaxations and MD runs with SIESTA
    ...
    return read_siesta_output_from_content(content, index, non_convergence_ok)
